...
TEST(TableCellRangeTests, CellRangeTest1) {
  ...
  for (int n = 0; n <= 5; n++) {
    ...
    std::stringstream ss;
    ss << rand;
    document->update_content_current_cells(ss.str(), generate_update_id());
  }
  ...
  document->update_content_current_cells("=(+ (cell_range 0 0 5 0))", generate_update_id());
...
TEST(TableCellRangeTests, RangeEqualityTest1) {
  ...
  for (int r = 0; r < 5; r++) {
    for (int c = 0; c < 10; c++) {
      ...
      document->update_content_current_cells("2", generate_update_id());
    }
  }
  ...
  document->update_content_current_cells("3", generate_update_id());
  ...
  document->update_content_current_cells("Hello", generate_update_id());
  EXPECT_EQ(cell->visible_content(), "FALSE");
}
